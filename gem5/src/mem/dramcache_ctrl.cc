	...
	cacheFillsThisTime(0), cacheWritesThisTime(0)
{
	...
	inform("RUNNING IN GPU BYPASS DRAMCACHE MODE\n");
...
DRAMCacheCtrl::addToReadQueue(PacketPtr pkt, unsigned int pktCount)
{
	...
	if ( (pktsServicedByWrQFillQ == 0) && (pkt->req->contextId() != 31) && !isInPamQueue)
	{
...
#endif
		{
			...
			Tick predReady = curTick() + PREDICTION_LATENCY;
			pamQueue[blk_addr]->mshr =
					allocateMissBuffer(clone_pkt, predReady, true);
		}
	}
#endif
...
DRAMCacheCtrl::recvTimingReq (PacketPtr pkt)
{
	...
	if (blocked != 0)
	{
		DPRINTF(DRAMCache,"%s cache blocked %d\n", __func__, pkt->getAddr());
	...
	if (pkt->isRead ())
	{
		// GPU request and GPU is running then bypass
...
DRAMCacheCtrl::recvTimingResp (PacketPtr pkt)
{
	...
	if(mshr->queue->index == 0 && pkt->isRead())
	{
		...
		// if GPU is running and response was for a GPU packet
		// check if the MSHR has a target from CPU
		// if MSHR has a target from CPU only then we perform fill, else bypass
		if(CudaGPU::running && (pkt->req->contextId()==31))
		{
			DPRINTF(DRAMCache,"Bypass packet recv for add %d\n", pkt->getAddr());
			fillRequired = false;
